...
const ChangeNode = () => {
  ...
  // Stored node metadata is migrated once when this settings page opens.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const sendChangeNodeEvent = (newNode) => {
    sendRuntimeMessage({
  ...
  const onSelectNode = async (node) => {
    ...
    try {
      ...
      await sendRuntimeMessage({
        ...
        data: { nodeUrl: node, chainId: selectedChainId },
      }, { fallback: null });
      if (previousChainId !== selectedChainId) {
